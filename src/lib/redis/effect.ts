...
import { Effect, Data, Context, Layer, Duration, Option } from 'effect';
...
const cacheGet = <T>(key: string) =>
  Effect.gen(function* () {
    ...
    Effect.catchAll((e) => Effect.zipRight(log.error(e), Effect.succeed(Option.none<T>())))
  );

const cacheSet = (key: string, value: unknown, ttl: DurationInput = Duration.minutes(10)) =>
  Effect.gen(function* () {
    const redis = yield* RedisService;
...
const cacheInvalidate = (pattern: string) =>
  Effect.gen(function* () {
    ...
    const keys = yield* Effect.tryPromise({
      ...
      catch: (cause) => new CacheError({ operation: 'invalidate', key: pattern, cause })
    });
    if (keys.length > 0) {
      yield* Effect.tryPromise({
        try: () => redis.del(...keys),
  ...
  Effect.gen(function* () {
    ...
    Effect.catchTag('CacheError', (e) =>
      Effect.zipRight(
        ...
        Effect.tryPromise({
          try: fetcher,
          catch: (cause) => new CacheError({ operation: 'fallback', key, cause })
        })
      )
    ),
    Effect.orDie
